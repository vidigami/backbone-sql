...
(function() {
  ...
  module.exports = SqlCursor = (function(_super) {
    ...
    SqlCursor.prototype.queryToJSON = function(callback) {
      ...
      if (this._cursor.$distinct) {
        $fields = this._cursor.$white_list ? _.intersection(this._cursor.$distinct, this._cursor.$white_list) : this._cursor.$distinct;
        if (this.hasCursorQuery('$count')) {
          return query.select(this.connection.raw("count(distinct " + ($fields.join(', ')) + ")")).exec((function(_this) {
            return function(err, count_json) {
              return callback(err, _extractCount(count_json));
            };
          })(this));
        }
      } else if (this._cursor.$values) {
