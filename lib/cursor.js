...
(function() {
  var COMPARATORS, COMPARATOR_KEYS, Cursor, Knex, SqlCursor, util, _, _appendCondition, _appendConditionalWhere, _appendSort, _appendWhere, _columnName,
        ...
        if (this.hasCursorQuery('$count')) {
          return query.count('*').exec((function(_this) {
            return function(err, json) {
              return callback(null, json.length ? +json[0].count : 0);
            };
          })(this));
        } else {
          return query.count('*').limit(1).exec((function(_this) {
            return function(err, json) {
              return callback(null, json.length ? !!+json[0].count : false);
            };
          })(this));
        }
      }
        ...
        return query.count('*').exec((function(_this) {
          return function(err, count_json) {
            ...
            if (count_json.length) {
              total_rows = +count_json[0][count_json[0].hasOwnProperty('count(*)') ? 'count(*)' : 'count'];
            }
