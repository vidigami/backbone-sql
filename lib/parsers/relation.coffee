...
module.exports = class RelationParser
  ...
  @parse: (from_model, raw_relations) ->
    ...
    for name, relation_options of raw_relations
      ...
      result[name] = @create_parsed_relation(name, relation_type, from_model, to_model, options)

    return result

  @create_parsed_relation: (name,  relation_type, from_model, to_model, options) ->
    type: relation_type
    model: to_model
