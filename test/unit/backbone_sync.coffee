...
describe 'BackboneSync', ->
  ...
  describe 'counts models', ->
    it 'counts by query', (done) ->
      ...
      queue.defer (callback) ->
        Photo.count {name: 'Bob'}, (err, count) ->
          assert.equal(count, 1, 'found Bob through query')
          callback(err)

      queue.defer (callback) ->
        Photo.count {name: 'Fred'}, (err, count) ->
  ...
  # sync: new BackboneSync({database_config: require('../config/database'), collection: 'bobs', model: Photo, manual_id: true, indices: {id: 1}})
